...
func (app *App) PromoteToRegionAdmin(c *gin.Context) {
	...
	//app.DB.First(&adminPermission, "where accsess_right = ?", "admin")
	app.DB.Preload("permissions").Preload("group_permissions").First(&group, "where permissions.acces_right = ?", "admin") //db.Group{Permissions: db.Permission{AccessRight: "admin"}})
